...
def plothitratetimedepend(datastoragelen, cachelenstart, steps, number):
    ...
    algs, x = [["lru"], ["lfu"], ["rlQ"], ["rlS"], ["slru"]], [number // steps * _ for _ in range(steps + 1) if _ != 0]
    ...
    plt.ylabel("Відсоток кеш-попадань")
    plt.legend(title="Алгоритм кешування", handles=[line1, line2, line5, line3, line4], numpoints=1)
    plt.show()